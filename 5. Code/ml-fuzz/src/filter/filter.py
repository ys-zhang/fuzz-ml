...
class Filter:
    ...
    def __init__(self):
        ...
        # a batch of samples
        self.samples_X = []  # type: List[np.ndarray]
        self.samples_Y = []  # type: List[np.ndarray]
    ...
    def sample_size(self) -> int:
        return sum(y.shape[0] for y in self.samples_Y)
    ...
    def need_fit(self) -> bool:
        ...
        #     return True
        if avg_score < self.model_train_max_score - model_train_score_span * 0.4:
            return True
    ...
    def _cast_samples(self):
        ...
        if not isinstance(self.samples_X, list):
            return
        self.samples_X = np.vstack(
            [np.concatenate(
                (x, np.zeros((x.shape[0], self.model_input_size - x.shape[1]))
                 ), axis=1
            ) for x in self.samples_X]
        )
        self.samples_Y = np.vstack(self.samples_Y)
    ...
    def print_filter_stat(self):
        ...
        print(f"new_obv_edge_sample_count/tot_sample_num: "
              f"{self.new_observed_edge_sample_count}/{self.total_sample_count}")
        print(f"oversize-ratio, sample-oversize-ratio:{self.model_oversize_input_ratio}, "
              f"{self.model_oversize_input_sample_ratio}")
    ...
    def observe_depre(self, x: np.ndarray, y: np.ndarray):
    ...
    def observe(self, x: np.ndarray, y: np.ndarray):
        nrow, ncol = x.shape
        if ncol > self.model_input_size:
            self.oversize_input_count += nrow
            self.longest_input_size = max(self.longest_input_size, ncol)
            # since this sample is dropped, we check whether it hits uncovered edges
            y_virgin = np.delete(y, self.edge_selector, axis=1)
            self.new_observed_edge_sample_count += np.any(y_virgin, axis=1).sum()
            # maintain edge statistics
            self.total_sample_count += nrow
    ...
    def filter_depre(self, inputs: List[bytes]) -> List[bytes]:

        with self.lock:
            if not self.model_ready:
                return inputs

            need_to_fuzz = [x for x in inputs if len(x) > self.model_input_size]
            need_to_pred = [x for x in inputs if len(x) <= self.model_input_size]
            x = np.array([np.concatenate((np.np.frombuffer(x, dtype=np.uint8),
                                          np.zeros(self.model_input_size - len(x))))
                          for x in need_to_pred])
            if need_to_pred:
                y_hat = self.predict(x)
                score = self.calc_score(y_hat, self.edge_score)
                # take the smallest 5% of inputs
                k = min(int(len(need_to_pred) * .05), 1)
                idx = np.argpartition(score, k)[:k]
                need_to_fuzz += np.take(need_to_pred, idx)

            return need_to_fuzz

    def filter(self, x: np.ndarray, x_len: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        if not self.model_ready:
            return x, x_len

        x = np.concatenate((x.astype(np.byte), np.zeros((x.shape[0], self.model_input_size - x.shape[1]))), axis=1)
        y_hat = self.predict(x.astype(np.byte))
        score = self.calc_score(y_hat, self.edge_score)
        k = min(int(len(x) * .05), 100)
        idx = np.argpartition(score, k)[:k]
        need_to_fuzz = np.take(x, idx, axis=0)
        need_to_fuzz_len = np.take(x_len, idx)
        return need_to_fuzz.astype(np.uint8), need_to_fuzz_len
